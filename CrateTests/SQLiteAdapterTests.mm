...
#define DEFAULT_CONNECT @{@"host": @"./crate_test.sqlite", @"database_name":@"crate_test"}

@interface SQLiteAdapterTests : XCTAsyncTestCase
@end

@implementation SQLiteAdapterTests {
    dispatch_queue_t dbQueue;
    SQLiteAdapter *dbConnection;
}
...
- (void)setUp
{
    ...
    if (!dbConnection) {
        dbConnection = [[SQLiteAdapter alloc] init];
    }
}
    ...
    [dbConnection connectWithDictionary:DEFAULT_CONNECT
                          dispatchQueue:dbQueue
                                ...
                                failure:^(NSString *error) {
                                    NSLog(@"Could not connect to SQLite with DEFAULT_CONNECT");
